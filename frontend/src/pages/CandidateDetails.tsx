...
const CandidateDetails: React.FC = () => {
  ...
  const handleStartVerification = async () => {
    ...
    try {
      ...
      setCandidate(res.data);
      alert('Background verification process completed successfully!');
    } catch (err: any) {
      alert(err.response?.data?.message || 'Verification pipeline encountered a network error.');
    } finally {
  ...
  return (
    <div className="flex-1 p-8 flex flex-col gap-6 max-w-5xl mx-auto">
      {/* Return button */}
      <div>
        <button
